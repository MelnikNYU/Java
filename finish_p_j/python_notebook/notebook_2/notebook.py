...
while enter != 4:
    ...
    if enter == 1:
        ...
        stroka = str(input("Введите название заметки "))
        ...
        stroka = str(input("Введите номер "))
        search(path, stroka)
...
def create(path):
    try:
        ...
        print('Создана новая заметка -> note_book.txt ')
        file = open(path, 'w')
    finally:
        file.close()
...
def find_char():
    print('Choose char: ')
    print('0 - id, 1 - name, 2 - number, s - spop')
    char = input()
...
def start ():
    ...
    try:
        ...
        print('New file created -> note_book.txt ')
        file = open(path, 'w')
    finally:
        file.close()